...
export function memberships(state = { items: [] }, action) {
  switch (action.type) {
    ...
    case membershipConstants.UPDATE_REQUEST:
      return {
        ...state,
        loading: true
      };
    case membershipConstants.UPDATE_SUCCESS:
      items = state.items;
      let id = items.findIndex(x => x.id === action.membership.id);
      items[id] = action.membership;
      return {
        ...state,
        items: items
      };
    case membershipConstants.UPDATE_FAILURE:
      return {
        ...state,
        error: action.error
      };
